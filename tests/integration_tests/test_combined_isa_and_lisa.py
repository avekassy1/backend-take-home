...
class TestIsaAllowanceCalculator(TestCase):
    ...
    def test_simple_isa_exhausting_20k_allowance_before_lifetime_isa(self):
        transactions = [
            Transaction(account=self.flex_account, amount=Decimal(25_000), transaction_date=dt.date(2024, 4, 26)),
            Transaction(account=self.lifetime_account, amount=Decimal(8_000), transaction_date=dt.date(2024, 4, 27)),
            Transaction(account=self.lifetime_account, amount=Decimal(-1_000), transaction_date=dt.date(2024, 4, 28)),
            Transaction(account=self.lifetime_account, amount=Decimal(-6_000), transaction_date=dt.date(2024, 4, 29)),
            Transaction(account=self.flex_account, amount=Decimal(-25000), transaction_date=dt.date(2024, 5, 1)),
        ]
        allowance = calculate_isa_allowance_for_account(client_id=self.client_id, transactions=transactions, tax_year=2024)
        assert allowance.annual_allowance == Decimal(20_000)
    ...
    def test_three_isas_together(self):
        transactions = [
            Transaction(account=self.nonflex_account, amount=Decimal(15_000), transaction_date=dt.date(2024, 4, 26)), # Uses 15k
            Transaction(account=self.lifetime_account, amount=Decimal(10_000), transaction_date=dt.date(2024, 4, 27)), # Uses 4k - 19k total -> 1k allowance left
            Transaction(account=self.nonflex_account, amount=Decimal(-5_000), transaction_date=dt.date(2024, 4, 27)), # Still 19k used
            Transaction(account=self.flex_account, amount=Decimal(2_000), transaction_date=dt.date(2024, 4, 28)), # 21k in total
            Transaction(account=self.lifetime_account, amount=Decimal(-8_000), transaction_date=dt.date(2024, 4, 29)), # Frees up 2k via lifetime acocount, 19k used in total
