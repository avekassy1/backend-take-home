...
class TestIsaAllowanceCalculator(TestCase):
    ...
    def test_simple_lifetime_isa_allowance_exhausted_then_reduced_back_below_limit(
        self,
    ):
        transactions = [
            Transaction(
            ...
            Transaction(
                account=self.lifetime_account,
                amount=Decimal(-2_000),
                transaction_date=dt.date(2024, 5, 6),
            ),
            Transaction(
                account=self.lifetime_account,
                amount=Decimal(1_500),
                transaction_date=dt.date(2024, 5, 7),
            ),
            Transaction(
                account=self.lifetime_account,
                amount=Decimal(-1_500),
                transaction_date=dt.date(2024, 5, 8),
            ),
        ]
        allowance = calculate_isa_allowance_for_account(
            client_id=self.client_id, transactions=transactions, tax_year=2024
