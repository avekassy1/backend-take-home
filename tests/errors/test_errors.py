...
class TestIsaAllowanceCalculator(TestCase):
    def setUp(self):
        self.client_id = 1
        self.implemented_account = Account(
            id=self.client_id, client_id=1, account_type=AccountType.Flexible_ISA
        )
        self.unimplemented_account = Account(
            id=self.client_id, client_id=1, account_type=AccountType.Not_Implemented_ISA
        )

    def test_simple_lifetime_isa_allowance_calculator_with_unordered_transactions(self):
        transactions = [
            Transaction(
                account=self.unimplemented_account,
        ...
        with pytest.raises(NotImplementedError):
            calculate_isa_allowance_for_account(
                client_id=1, transactions=transactions, tax_year=2024
            )

    def test_withdrawals_creating_negative_account_balance(self):
        transactions = [
            Transaction(
                account=self.implemented_account,
                amount=Decimal(5_000),
                transaction_date=dt.date(2024, 5, 10),
            ),
            Transaction(
                account=self.implemented_account,
    ...
    def test_year_other_than_2024(self):
        with pytest.raises(NotImplementedError):
            calculate_isa_allowance_for_account(
                client_id=1, transactions=[], tax_year=2020
            )
